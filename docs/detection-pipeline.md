...
|---|---|---|---|
| 1 | Contexto e regras locais (texto, rótulos, URLs) | Content script | **Implementada** |
...
interface TextRule {
  ...
  fields: ContextField[];            // title | description | hashtags | ariaLabels | captions | authorName | pageUrl
  weight: number;
  confidence: number;
...
- **Declarações explícitas** (`synthetic_media`, grupo `explicit-declaration`, weight 0.85, confidence 0.9): "gerado por IA", "criado com IA", "conteúdo sintético", "AI generated", "synthetic media", "generated with", "vídeo de IA".
- **Hashtags** (`synthetic_media`, grupo `ai-hashtag`, weight 0.45, confidence 0.85): `#ia`, `#ai`, `#aigenerated`, `#geradoporia`, `#aivideo`, `#sora` etc.
- **Rótulo da plataforma** (`platform_disclosure`, grupo `platform-label`, weight 0.95, confidence 0.95): produzido pelos adaptadores quando o DOM expõe o selo nativo (Fase 2; o adaptador genérico não gera este tipo).
...
- `assessmentVersion` começa em `0.1.0`; qualquer mudança em thresholds/agregação incrementa minor e exige atualização deste documento e dos testes.
